...
const ThanksView: React.FC<ThanksViewProps> = ({ onFinish, isRemote = false, reviewId }) => {
  ...
  return (
    <div className={`text-center flex flex-col items-center px-10 ${isRemote ? 'py-8' : 'py-4'} animate-fade-in`}>
      ...
      <div className={`${isRemote ? 'w-20 h-20 mb-8' : 'w-32 h-32 mb-10'} bg-white rounded-3xl flex items-center justify-center shadow-[0_20px_60px_rgba(255,255,255,0.1)] relative animate-fade-in-up`}>
        <div className="absolute -top-3 -right-3 w-10 h-10 bg-white rounded-xl shadow-lg flex items-center justify-center border-4 border-black">
           <span className="text-black text-sm font-black">✓</span>
        </div>
        <svg xmlns="http://www.w3.org/2000/svg" className={`${isRemote ? 'h-10 w-10' : 'h-16 w-16'} text-black`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div className="overflow-hidden mb-6">
        <h2 className={`${isRemote ? 'text-4xl' : 'text-6xl'} font-[900] text-white uppercase tracking-tight leading-none animate-slide-reveal`}>
          REVIEW <span className="text-slate-400">POSTED!</span>
        </h2>
      </div>
      ...
      <div className="flex flex-col items-center gap-8 mb-12 animate-fade-in-up" style={{ animationDelay: '0.6s' }}>
        ...
        <div className="space-y-3">
          <h4 className="text-white text-lg font-[900] uppercase tracking-wide">SCAN TO GET YOUR PHOTOS</h4>
          <p className="text-slate-500 text-[11px] font-bold uppercase tracking-[0.3em]">Official Expo Gallery Access</p>
        </div>
      </div>

      <div className={`${isRemote ? 'w-64' : 'w-[450px]'} h-3 bg-white/10 rounded-full overflow-hidden relative border border-white/10 p-0.5 animate-fade-in`} style={{ animationDelay: '0.8s' }}>
        <div className="h-full bg-white origin-left animate-loading-bar rounded-full shadow-[0_0_15px_rgba(255,255,255,0.4)]"></div>
      </div>
      ...
      {isRemote && (
        <button
          onClick={onFinish}
          className="mt-12 px-16 py-6 bg-white text-black rounded-2xl font-black shadow-2xl hover:scale-105 active:scale-95 transition-all uppercase tracking-tighter"
        >
          SUBMIT ANOTHER
        </button>
      )}
      ...
      {!isRemote && (
        <button
          onClick={onFinish}
          className="mt-12 px-8 py-4 text-slate-500 font-black hover:text-white transition-all text-[11px] uppercase tracking-[0.2em] flex items-center gap-4 group"
        >
          <div className="w-2.5 h-2.5 bg-white rounded-sm group-hover:rotate-180 transition-transform duration-500"></div>
          RETURN TO HOME
        </button>
      )}
    </div>
